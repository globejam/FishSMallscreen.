...
fishBlop = loadSound("./assets/161628__crazyfrog249__blop.wav");
}
...
function setup() {
 ...
 // fishingRod = createSprite(windowWidth/2+200,windowHeight/2-200,10,10);
  fisherman = createSprite(width/2,height-50,10,10);
...
//console.log(fishFrameCount);
 ...
 background("#1221c1");
  ...
  image(deepSea, 0, height, width, height);
  rodLength()
  skake();
  fish(0,1200);
 ...
 drawSprites();
 textSize(30)
  text("Score: "+score,width-200,height/2+560);
 showLife();
 ChangeFrameCount();
}
...
function rodLength(){
  ...
  if(keyIsDown(DOWN_ARROW)&& fLineLength<=height/2+250){
    fLineLength +=2
  }
   rect(fisherman.x-130,height-275,2,fLineLength);

 hookPos = height-275+fLineLength
...
function fish(x,y){
  ...
  var fish = createSprite(x,Math.round(random((height+200),(height+450))),80,80);
  fishGroup.add(fish);
  fish.scale = 0.5;
...
function skake(){
 ...
 function showLife() {
  ...
  image(lifeImage, width-230,height/2+580, 20, 20);
  fill("white");
  rect(width-200, height/2+580, 185, 20);
  fill("#f50057");
  rect(width-200,height/2+580, life, 20);
  noStroke(); 
}
